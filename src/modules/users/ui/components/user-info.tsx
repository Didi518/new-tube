...
const userInfoVariance = cva('flex items-center gap-1', {
  variants: {
    size: {
...
interface UserInfoProps extends VariantProps<typeof userInfoVariance> {
  name: string;
  className?: string;
}
...
export const UserInfo = ({ name, className, size }: UserInfoProps) => {
  return (
    <div className={cn(userInfoVariance({ size, className }))}>
      <Tooltip>
